...
// like *>
fn then<A: 'static, B: 'static, E: 'static>(
    a: Box<Parser<A, E>>,
    b: Box<Parser<B, E>>,
) -> Box<Parser<B, E>> {
    Box::new(move |inp: &str| {
...
// like <$>
fn map<A: 'static, B: 'static, E: 'static>(
    a: Box<Parser<A, E>>,
    f: fn(A) -> B,
) -> Box<Parser<B, E>> {
    Box::new(move |inp: &str| match a.call((inp,)) {
...
// like $>
fn p_as<A: 'static, B: 'static + Copy, E: 'static>(
    a: Box<Parser<A, E>>,
    b: B,
) -> Box<Parser<B, E>> {
    Box::new(move |inp: &str| match a.call((inp,)) {
...
fn p_str(s: String) -> Box<Parser<String, ParseError>> {
    Box::new(move |inp: &str| match inp.strip_prefix(&s.to_string()) {
...
fn p_or<A: 'static>(
    left: Box<Parser<A, ParseError>>,
    right: Box<Parser<A, ParseError>>,
) -> Box<Parser<A, ParseError>> {
    Box::new(move |inp: &str| {
        // try left branch
        match left.call((inp,)) {
            Ok(a) => Ok(a),
...
mod tests {
    ...
    fn it_works() {
        ...
        let full = then(char, p_str("ello".to_string())).call(("hello",));
        println!("{:?}", full)
    }
}
